...
int main(){
	int i, j, t = 1;
	double a[1005], b[1005], c[1005], x1, y1, x2, y2, aobj, bobj, ans;
	char sign, ineq[1005];
	vector<PT> poli;
	...
	while(scanf("%d %d %d", &E, &D, &N), E){
		...
		if(i < N)
			printf("Case #%d: No hay Región Factible\n", t);
		else{
			printf("Case #%d:La Región Factible es el polígono: \n", t);
			ans = INF;
			pans = poli[0];
			for(i = 0; i < poli.size(); i++){
				printf("(%.3lf, %.3lf) ", poli[i].x, poli[i].y);
				if(aobj * poli[i].x + bobj * poli[i].y < ans){
					ans = aobj * poli[i].x + bobj * poli[i].y;
					pans = poli[i];
				}
			}
			printf("\nEl mínimo es: %.6lf y se da en x = %.3lf, y = %.3lf\n", ans, pans.x, pans.y); 
		t++;
		}
	}
